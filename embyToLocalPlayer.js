// embyToLocalPlayer for Emby JavaScript Extension
// Modified to work without Tampermonkey/Greasemonkey
// Original: https://github.com/kjtsune/embyToLocalPlayer
'use strict';
/*global ApiClient*/
...
(function () {
    ...
    function _init_config_main() {
        function _init_config_by_key(confKey) {
            ...
            if (confLocal == 'true') {
                localStorage.setItem(confKey, 'true');
            } else if (confLocal == 'false') {
                localStorage.setItem(confKey, 'false');
            }
            let confValue = localStorage.getItem(confKey);
            if (confValue !== null) { config[confKey] = (confValue === 'true'); }
        }
        _init_config_by_key('crackFullPath');
    }
 
    // Menu functions removed for Emby JavaScript Extension compatibility
    // Settings can be configured via localStorage:
    // - webPlayerEnable: 'true' or 'false'
    // - mountDiskEnable: 'true' or 'false'
    // - crackFullPath: 'true' or 'false'
    ...
    // Set default value for webPlayerEnable if not set
    if (localStorage.getItem('webPlayerEnable') === null) {
        localStorage.setItem('webPlayerEnable', 'false');
        logger.info('webPlayerEnable set to default: false (external player enabled)');
    }
    
    // Settings info: Configure via localStorage
    // webPlayerEnable: 'true' to use web player, 'false' to use external player (default)
    // mountDiskEnable: 'true' to enable mount disk mode
    // crackFullPath: 'true' to show full path
    logger.info('embyToLocalPlayer initialized for Emby JavaScript Extension');
})();